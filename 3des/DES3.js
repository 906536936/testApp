 ...
 * DES 加密算法
 *
 * 该函数接受一个 8 字节字符串作为普通 DES 算法的密钥（也就是 64 位，但是算法只使用 56 位），
 * 或者接受一个 24 字节字符串作为 3DES算法的密钥；
 * 
 * 第二个参数是要加密或解密的信息字符串；
 * 第三个布尔值参数用来说明信息是加密还是解密；
 * 接下来的可选参数 mode 如果是 0 表示 ECB * 模式，1 表示 CBC 模式，默认是 ECB 模式；
 * 最后一个可选项是一个 8 字节的输入向量字符串（在 ECB 模式下不使用）。返回的密文是字符串。
 *
 * 参数： <br>
